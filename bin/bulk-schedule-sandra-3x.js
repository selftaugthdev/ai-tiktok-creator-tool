#!/usr/bin/env node
'use strict';
/**
 * Schedule Sandra carousels (no variant suffix) to TikTok via PostBridge.
 *
 * 3 posts per day — morning (08–10 Brussels), afternoon (13–15 Brussels), evening (19–21 Brussels).
 * Brussels is CEST (UTC+2) in May, so windows in UTC:
 *   Morning:   06:00–08:00 UTC
 *   Afternoon: 11:00–13:00 UTC
 *   Evening:   17:00–19:00 UTC
 *
 * Hashtags and CTA are force-applied on every post, overriding what's in caption.txt.
 *
 * Usage:
 *   node bin/bulk-schedule-sandra-3x.js
 *   node bin/bulk-schedule-sandra-3x.js --start 2026-05-19
 *   node bin/bulk-schedule-sandra-3x.js --start 2026-05-19 --count 21
 *   node bin/bulk-schedule-sandra-3x.js --dry-run
 */

require('dotenv').config();

const fs   = require('fs');
const path = require('path');

const TO_UPLOAD_DIR = 'output/to-upload/MigraineCast/tiktok/sandra/to_upload';
const API           = 'https://api.post-bridge.com';

// Only schedule carousels with numbers >= this to exclude older batches
const MIN_CAROUSEL_NUM = 176;

const FORCED_HASHTAGS = '#migraine #migrainerelief #migrainerelieftok #chronicmigraine #migrainewarrior';
const FORCED_CTA      = 'Stay ahead of your migraines with the MigraineCast app, link in Bio, free to download in the App store or at www.migrainecast.app';

// Staggered times within each UTC window (7 slots, cycling across days)
const MORNING_TIMES_UTC = [
  '06:15', '07:05', '06:45', '07:30', '06:05', '07:15', '06:35',
];
const AFTERNOON_TIMES_UTC = [
  '11:20', '12:10', '11:45', '12:35', '11:05', '12:20', '11:50',
];
const EVENING_TIMES_UTC = [
  '17:20', '18:05', '17:45', '18:30', '17:05', '18:15', '17:35',
];

// ── Helpers ───────────────────────────────────────────────────────────────────

function apiHeaders() {
  return {
    Authorization: `Bearer ${process.env.POSTBRIDGE_API_KEY}`,
    'Content-Type': 'application/json',
  };
}

function addDays(dateStr, n) {
  const d = new Date(`${dateStr}T00:00:00Z`);
  d.setUTCDate(d.getUTCDate() + n);
  return d.toISOString().slice(0, 10);
}

function utcToBrussels(utcTime) {
  const [h, m] = utcTime.split(':').map(Number);
  return `${String((h + 2) % 24).padStart(2, '0')}:${String(m).padStart(2, '0')}`;
}

/** Parse TITLE / DESCRIPTION / HASHTAGS sections from caption.txt */
function parseCaption(text) {
  const titleMatch = text.match(/^TITLE:\s*\n([\s\S]*?)(?=\n(?:DESCRIPTION|HASHTAGS):|$)/m);
  const descMatch  = text.match(/^DESCRIPTION:\s*\n([\s\S]*?)(?=\n(?:TITLE|HASHTAGS):|$)/m);

  const title       = titleMatch ? titleMatch[1].trim() : '';
  let   description = descMatch  ? descMatch[1].trim()  : text.trim();

  // Force-append CTA if not already present
  if (!description.includes(FORCED_CTA)) {
    description = description + '\n\n' + FORCED_CTA;
  }

  // Always use the forced hashtags (overrides whatever is in caption.txt)
  return { title, description, hashtags: FORCED_HASHTAGS };
}

/** Upload a single file, with up to 5 retries (catches both HTTP errors and socket errors). */
async function uploadFile(filePath, attempt = 1) {
  const stats    = fs.statSync(filePath);
  const fileName = path.basename(filePath);

  try {
    const urlRes = await fetch(`${API}/v1/media/create-upload-url`, {
      method:  'POST',
      headers: apiHeaders(),
      body:    JSON.stringify({ name: fileName, mime_type: 'image/png', size_bytes: stats.size }),
    });
    if (!urlRes.ok) {
      const err = await urlRes.json().catch(() => ({}));
      throw new Error(`Upload URL failed for ${fileName}: ${JSON.stringify(err)}`);
    }
    const { media_id, upload_url } = await urlRes.json();

    const putRes = await fetch(upload_url, {
      method:  'PUT',
      headers: { 'Content-Type': 'image/png' },
      body:    fs.readFileSync(filePath),
    });
    if (!putRes.ok) throw new Error(`PUT failed for ${fileName}`);

    return media_id;
  } catch (err) {
    if (attempt < 6) {
      const delay = attempt * 3000;
      process.stdout.write(` (retry ${attempt}, waiting ${delay / 1000}s...)`);
      await new Promise(r => setTimeout(r, delay));
      return uploadFile(filePath, attempt + 1);
    }
    throw err;
  }
}

async function createPost(payload) {
  const res  = await fetch(`${API}/v1/posts`, {
    method:  'POST',
    headers: apiHeaders(),
    body:    JSON.stringify(payload),
  });
  const json = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(`Create post failed: ${JSON.stringify(json)}`);
  return json;
}

function moveToUploaded(folderPath) {
  const abs  = path.resolve(folderPath);
  const dest = abs
    .replace(`${path.sep}to-upload${path.sep}`, `${path.sep}uploaded${path.sep}`)
    .replace(`${path.sep}to_upload${path.sep}`, `${path.sep}uploaded${path.sep}`);
  fs.mkdirSync(path.dirname(dest), { recursive: true });
  fs.renameSync(abs, dest);
}

// ── Main ──────────────────────────────────────────────────────────────────────

async function main() {
  const args      = process.argv.slice(2);
  const dryRun    = args.includes('--dry-run');
  const startIdx  = args.indexOf('--start');
  const countIdx  = args.indexOf('--count');
  const startDate = startIdx !== -1 ? args[startIdx + 1] : '2026-05-19';
  const maxCount  = countIdx !== -1 ? parseInt(args[countIdx + 1], 10) : 21;

  const key      = process.env.POSTBRIDGE_API_KEY;
  const tiktokId = parseInt(process.env.POSTBRIDGE_TIKTOK_ACCOUNT_ID, 10);

  if (!dryRun) {
    if (!key)            { console.error('POSTBRIDGE_API_KEY not set in .env');            process.exit(1); }
    if (isNaN(tiktokId)) { console.error('POSTBRIDGE_TIKTOK_ACCOUNT_ID not set in .env'); process.exit(1); }
  }

  // Collect qualifying folders: no -vN suffix, carousel number >= MIN_CAROUSEL_NUM
  const allFolders = fs.readdirSync(TO_UPLOAD_DIR)
    .filter(f => {
      if (!fs.statSync(path.join(TO_UPLOAD_DIR, f)).isDirectory()) return false;
      if (/-v\d+$/.test(f)) return false;                          // skip variants
      const num = parseInt(f.match(/carousel_(\d+)/)?.[1] ?? '0', 10);
      return num >= MIN_CAROUSEL_NUM;
    })
    .sort((a, b) => {
      const numA = parseInt(a.match(/carousel_(\d+)/)?.[1] ?? '0', 10);
      const numB = parseInt(b.match(/carousel_(\d+)/)?.[1] ?? '0', 10);
      return numA - numB;
    })
    .slice(0, maxCount);

  if (allFolders.length === 0) {
    console.error(`No eligible carousels found in ${TO_UPLOAD_DIR}`);
    process.exit(1);
  }

  const totalDays = Math.ceil(allFolders.length / 3);
  const endDate   = addDays(startDate, totalDays - 1);

  console.log(`\nFound ${allFolders.length} carousels to schedule (max: ${maxCount})`);
  console.log(`Start date:  ${startDate}`);
  console.log(`End date:    ${endDate}  (${totalDays} days)`);
  console.log(`Platform:    TikTok`);
  console.log(`Schedule:    3/day — morning 08–10, afternoon 13–15, evening 19–21 Brussels (UTC+2)`);
  console.log(`Hashtags:    ${FORCED_HASHTAGS}`);
  if (dryRun) console.log('\n⚠️  DRY RUN — no API calls will be made\n');
  else        console.log('');

  const SLOT_TIMES = [MORNING_TIMES_UTC, AFTERNOON_TIMES_UTC, EVENING_TIMES_UTC];
  const SLOT_NAMES = ['morning', 'afternoon', 'evening'];

  let posted = 0;
  let errors = 0;

  for (let i = 0; i < allFolders.length; i++) {
    const dayIndex  = Math.floor(i / 3);
    const slotIndex = i % 3;
    const timeUtc   = SLOT_TIMES[slotIndex][dayIndex % SLOT_TIMES[slotIndex].length];
    const dateStr   = addDays(startDate, dayIndex);
    const scheduledAt = `${dateStr}T${timeUtc}:00Z`;
    const slotName  = SLOT_NAMES[slotIndex];

    const folder      = path.join(TO_UPLOAD_DIR, allFolders[i]);
    const captionPath = path.join(folder, 'caption.txt');

    const slides = fs.readdirSync(folder)
      .filter(f => /^slide_\d+\.png$/.test(f))
      .sort();

    if (!fs.existsSync(captionPath) || slides.length === 0) {
      console.log(`  ⚠️  Skipping ${allFolders[i]} — missing caption.txt or slides`);
      errors++;
      continue;
    }

    const { title, description, hashtags } = parseCaption(fs.readFileSync(captionPath, 'utf8'));
    const caption = [description, hashtags].filter(Boolean).join('\n\n');

    console.log(`\n[${i + 1}/${allFolders.length}] ${allFolders[i]}`);
    console.log(`  ${dateStr}  ${timeUtc} UTC (${utcToBrussels(timeUtc)} Brussels)  [${slotName}]`);
    console.log(`  Title: ${title}`);
    console.log(`  Slides: ${slides.length}`);

    if (dryRun) { posted++; continue; }

    try {
      const mediaIds = [];
      for (const slide of slides) {
        process.stdout.write(`  Uploading ${slide}...`);
        const mediaId = await uploadFile(path.join(folder, slide));
        mediaIds.push(mediaId);
        console.log(' ✓');
        await new Promise(r => setTimeout(r, 300));
      }

      process.stdout.write('  Creating TikTok post...');
      const post = await createPost({
        caption,
        social_accounts: [tiktokId],
        media:           mediaIds,
        scheduled_at:    scheduledAt,
        platform_configurations: {
          tiktok: { title, caption, draft: true },
        },
      });
      console.log(` ✓ (id: ${post.id})`);

      moveToUploaded(folder);
      posted++;
    } catch (err) {
      console.error(`\n  ✗ Error: ${err.message}`);
      errors++;
    }

    await new Promise(r => setTimeout(r, 500));
  }

  console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`Done! ${posted} scheduled to TikTok, ${errors} errors.`);
  console.log(`Spread across ${totalDays} days (${startDate} → ${endDate})`);
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
}

main().catch(err => { console.error(err); process.exit(1); });
